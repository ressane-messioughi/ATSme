// Contrepartie présentationnelle de frontend/src/pages/AtsAnalysis.tsx pour le design
...
import { scoreTone, scoreLabel, type Resume, type ResumeSummary } from "./types.js";
import { sampleResume, sampleResumeList } from "./sampleData.js";
import { IconCheck, IconWarn } from "./icons.js";
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      {ratios.map((_, i) => {
        ...
        return <line key={i} x1={cx} y1={cy} x2={cx + radius * Math.cos(angle)} y2={cy + radius * Math.sin(angle)} stroke="var(--border)" strokeWidth="1" />;
      })}
...
export type AtsAnalysisScreenProps = {
  /** CV analysé en détail. */
  resume?: Resume;
  /** Liste proposée dans le sélecteur — changer la sélection ne recharge pas de vraies
   * données dans cette version présentationnelle, `resume` reste affiché tel quel. */
  resumes?: ResumeSummary[];
};

/**
 * Analyse ATS détaillée d'un CV : jauge de score, radar par catégorie, points forts et
 * recommandations.
 *
 * @example
 * <AtsAnalysisScreen />
 */
export function AtsAnalysisScreen({ resume = sampleResume, resumes = sampleResumeList }: AtsAnalysisScreenProps) {
  const [selectedId, setSelectedId] = useState(String(resume.id));
    ...
    <div className="max-w-5xl">
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6 flex flex-col items-center justify-center text-center">
          <ScoreGauge score={resume.atsScore} size={140} tone={scoreTone(resume.atsScore)} label={scoreLabel(resume.atsScore)} />
      ...
      <div className="flex justify-center mt-6">
        <button className="bg-[var(--violet)] hover:bg-[var(--violet-soft)] transition-colors rounded-lg px-5 py-2.5 text-sm font-medium text-white cursor-pointer">
